...
/**
 * Community object to collect all discussion objects and user objects involved
 * in the interaction of a community.
 * 
...
public class Community {
	...
	/**
	 * Default label for accounts for which no name is known
	 */
	public final static String ANONYMOUS_ACCOUNT = "(anonymous)";

	/**
	 * A name for the community (for displaying purposes)
	 */
	...
	public TreeSet<DiscussionNode> getUserContributions(User user) {
		...
		if (this.contributions.containsKey(user.getId())) {
			TreeSet<Long> contributionIds = this.contributions
					.get(user.getId());
	...
	/**
	 * Adds the user to the list of users. If a user with the same id exists it
	 * is overwritten.
	 * 
	 * @param user
	 */
	...
	/**
	 * Updates the references of users to contributions when the user of a node
	 * has been changed.
	 * 
	 * @param nodeId
	 * @param oldUser
	 * @param newUser
	 */
	protected void updateUserContributions(long nodeId, User oldUser,
			User newUser) {
		if (oldUser != null) {
